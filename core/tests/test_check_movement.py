...
def pastor():
        ...
        board.bypass_validation_move("f1c4")  
        board.bypass_validation_move("d1f3")  
        board.white = "jose"
        board.black = "mike"
        return board
...
def test_king_move_out_check(king_in_check_by_bishop: Board):
    ...
    assert king_in_check_by_bishop.winner == ""
...
def test_invalid_king_goes_to_check(board5: Board):
    ...
    assert board5.winner == ""

def test_check_mate(pastor: Board):
    pastor.move("f3f7")
    assert pastor.legal == True
    assert pastor.winner == "jose"